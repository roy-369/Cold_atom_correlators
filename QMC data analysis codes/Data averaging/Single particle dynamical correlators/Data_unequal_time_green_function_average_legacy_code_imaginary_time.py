...
def unequal_time_greens_function_real_space_average(Text_dir_gf_r,Text_dir_gf_r_avg_ac,N,U,Mu,dtau,L,run_no):
   ...
   for rr in range(int(run_no)):
       r0 = str(rr)

       filename_gf_r_0 = '%s/Retarded_Green_function_real_space_N_%s_U_%s_mu_%s_dtau_%s_L_%s_r_%s.pkl' %(Text_dir_gf_r,N,U,Mu,dtau,L,r0)
       print("ru no",rr)
   ...
   if os.path.exists(filename_gf_r_0):
       ...
       filename_gf_r_variation_0 = '%s/Retarded_Green_function_real_space_standard_deviation_N_%s_U_%s_mu_%s_dtau_%s_L_%s_r_%s.pkl' %(Text_dir_gf_r,N,U,Mu,dtau,L,r0)
       ...
       for rr in range(r_points):
           ...
           for tt in range(len(G_r_std_data)):
               if np.abs(G_r_std_data[tt]) < 0.000001:
                  G_r_std_data[tt] = 0.000001
...
def unequal_time_greens_function_momentum_space_average(Text_dir_gf_k,Text_dir_gf_k_avg_ac,N,U,Mu,dtau,L,n_den,run_no):
   ...
   for rr in range(int(run_no)):
       ...
       filename_gf_k_0 = '%s/Retarded_Green_function_momentum_space_N_%s_U_%s_mu_%s_dtau_%s_L_%s_r_%s.pkl' %(Text_dir_gf_k,N,U,Mu,dtau,L,r0)
       print("run_no",rr)
   ...
   if os.path.exists(filename_gf_k_0):
       ...
       filename_gf_k_variation_0 = '%s/Retarded_Green_function_momentum_space_standard_deviation_N_%s_U_%s_mu_%s_dtau_%s_L_%s_r_%s.pkl' %(Text_dir_gf_k,N,U,Mu,dtau,L,r0)
       ...
       for kk in range(k_points):
           ...
           for tt in range(len(G_k_std_data)):
               if np.abs(G_k_std_data[tt]) < 0.000001:
                  G_k_std_data[tt] = 0.000001
...
def main(total,cmdargs):
    ...
    Text_dir_gf_r_avg_ac = '/fs/byo/trivedi/roy.369/Research/Determinant_Quantum_Monte_Carlo/UC_code/legacy-qmc/Cold_atom_correlators/Text_files/Text_files_N_%s_analytic_continuation/Text_files_N_%s_U_%s_dtau_%s/Mu_%s/dtau_%s_L_%s/Retarded_green_functions_real_space_averaged'%(N,N,U,Dtau,mu,Dtau,L)
